...
test('dashboard exposes a distinct assistant surface for managers and workers', () => {
  ...
  const payrollHelp = readFileSync(new URL('../lib/payroll-help.ts', import.meta.url), 'utf8');
  ...
  assert.match(dashboard, /assistantPayrollSuggestions\(role\)/);
  assert.match(dashboard, /PayrollAskQuestionnaire/);
